...
# one unique K=0.5 assimilated prior trajectory per year because both independent training folds selected the same gain
...
emp=['dap','gdd_cum','LV','GF','Tmean','ETo','Rain','Irr','irr7','irr14','rain7','rain14','eto7','eto14','days_since_irr','cum_irr','cum_rain','cum_eto'];proc_extra=['Canopy_AC','z_root_m','Es_mm','Tr_mm','DeepPerc_mm']
...
for ty,vy in [(2012,2013),(2013,2012)]:
 ...
 # Full digital twin: residual AI is retrained on causal PRIOR state predictions that already contain only earlier sensor updates.
...
# stage-specific full-model metrics on each held-out year (not sensor VOI; simply model performance by development stage)
for key in ['2012->2013','2013->2012']:
 d=pd.read_csv(OUT+'/blind_predictions.csv');d=d[d.fold==key];R[key]['stage_performance']={}
 for s,g in d.groupby('stage_group'):
  R[key]['stage_performance'][s]={'n':len(g),'M0_RMSE':met(g.SWD_obs,g.M0_AquaCrop)['RMSE'],'M3_RMSE':met(g.SWD_obs,g.M3_OOD_AI)['RMSE'],'M4_RMSE':met(g.SWD_obs,g.M4_full)['RMSE']}
