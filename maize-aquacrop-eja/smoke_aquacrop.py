...
OUT=ROOT/'fcinit_outputs'; OUT.mkdir(exist_ok=True)
...
def run(year,trt):
    ...
    initial=InitialWaterContent(wc_type='Prop',method='Layer',depth_layer=[1,2,3,4,5,6,7],value=['FC','FC','FC','FC','FC','FC','FC'])
    model=AquaCropModel(plant.strftime('%Y/%m/%d'),harvest.strftime('%Y/%m/%d'),build_weather(year),soil_for(year,trt),crop,initial,irrigation_management=irr)
    ...
    return {'year':year,'trt':trt,'plant':str(plant.date()),'harvest':str(harvest.date()),'params':p,'final':st.iloc[0].replace({np.nan:None}).to_dict()}

summary=[]
for year in (2012,2013):
    for trt in range(1,13):
        print(f'Running AquaCrop full-profile FC {year} treatment {trt}')
