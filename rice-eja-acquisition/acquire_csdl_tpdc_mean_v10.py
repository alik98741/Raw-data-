...
import json,urllib.request,ftplib,re,csv,hashlib,zipfile
...
# Exact verified 34 ADM1 -> NBS31 crosswalk; Taiwan/HK/Macau intentionally excluded by null mapping.
...
g['province_raw']=g[namecol].astype(str);g['province']=g.province_raw.map(CROSSWALK);g=g[g.province.notna()].copy()
...
inventory=[]
for p in PROPS:
 ftp.cwd('/1km/GTiff/'+p);ls=[(n,facts) for n,facts in ftp.mlsd() if n not in('.','..') and facts.get('type')=='file']
 for d in DEPTHS:
...
for k,item in enumerate(inventory,1):
 ...
 zpath=WORK/fn;h=hashlib.sha256();counter=[0];ftp.voidcmd('TYPE I')
 with open(zpath,'wb') as f:
  def cb(b):counter[0]+=len(b);h.update(b);f.write(b)
 ...
 with rasterio.open(tpath) as src:
  ...
  ma=src.read(1,masked=True);raw=np.asarray(ma.filled(np.nan),dtype='float64');scale=float(src.scales[0]) if src.scales else 1.0;offset=float(src.offsets[0]) if src.offsets else 0.0;data=raw*scale+offset;valid=np.isfinite(data)
  ...
  srcmeta={'crs':str(src.crs),'width':src.width,'height':src.height,'nodata':src.nodata,'scale':scale,'offset':offset,'unit':units,'description':desc,'tags_json':json.dumps(tags,ensure_ascii=False),'band_tags_json':json.dumps(bandtags,ensure_ascii=False)}
  for i,rec in gg.iterrows():
   vals=data[(labels==(i+1))&valid]
   if vals.size:
    q05,q50,q95=np.quantile(vals,[.05,.5,.95]);rows.append({'province':rec.province,'property':p,'depth':d,'n_pixels':int(vals.size),'mean':float(vals.mean()),'spatial_q05':float(q05),'spatial_q50':float(q50),'spatial_q95':float(q95),'spatial_sd':float(vals.std(ddof=1)) if vals.size>1 else np.nan,'min':float(vals.min()),'max':float(vals.max()),'unit':units,'scale':scale,'offset':offset})
 manifest.append({**item,'download_bytes':n,'sha256':h.hexdigest(),'internal_tif':tifs[0],**srcmeta});tpath.unlink(missing_ok=True);zpath.unlink(missing_ok=True)
try:ftp.quit()
except:pass
long=pd.DataFrame(rows);long.to_csv(OUT/'CSDLv2_1km_mean_ADM1_depth_long.csv',index=False);pd.DataFrame(manifest).to_csv(OUT/'CSDLv2_1km_mean_DOWNLOAD_SHA256_MANIFEST.csv',index=False)
...
prof=pd.concat([weighted(30,'0-30cm'),weighted(60,'0-60cm'),weighted(100,'0-100cm')],ignore_index=True);prof.to_csv(OUT/'CSDLv2_1km_mean_ADM1_depth_weighted_profiles.csv',index=False)
wide=prof.pivot_table(index=['province','profile'],columns='property',values='mean').reset_index();wide.columns=[str(c) for c in wide.columns];wide.to_csv(OUT/'CSDLv2_1km_mean_ADM1_model_ready_means.csv',index=False)
status={'source':'TPDC official no-login FTP','dataset_uuid':UUID,'share_policy':'A','ftp_server_used':host,'ftp_port':port,'files_downloaded':len(manifest),'expected_files':54,'properties':PROPS,'depths':DEPTHS,'profiles':['0-30cm','0-60cm','0-100cm'],'boundary':'geoBoundaries CHN ADM1 exact verified NBS31 crosswalk','important_notes':['These are CSDLv2 1-km MEAN prediction rasters.','spatial_q05/q50/q95 are within-province spatial pixel distributions, NOT CSDL model prediction quantile layers.','Rice-area-specific weighting is not claimed here; these are province polygon summaries pending a complete validated rice-support mask.'],'outputs':[p.name for p in OUT.iterdir()]}
(OUT/'CSDLv2_TPDC_MEAN_ACQUISITION_STATUS.json').write_text(json.dumps(status,indent=2),encoding='utf-8');print(json.dumps(status,indent=2))
